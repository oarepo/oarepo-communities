...
We only need to pin the ``INVENIO_COLLECTIONS_COMMUNITY_SLUG`` config key that
the collections records service reads and materialize a holder community, a
"different" community, and a tree + collection through the actual service.
"""
...
def app_config(app_config: dict[str, Any]) -> dict[str, Any]:
    """Pin the config key the collections records service reads."""
