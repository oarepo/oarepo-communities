...
def _check_community_submission_and_publish(creator_client, receiver_client, receiver, record,  community):
    submission_request = creator_client.post(f"/communities/{community}/records/{record}")
    request = current_requests_service.read(receiver.identity, submission_request.json['id'])
...
def test_owner(
    ...
    record_resp = _create_and_publish(owner_client, input_data, community_1)
    assert record_resp.status_code == 202
    ...
    # test standard crud
    rec_id = _create_and_publish(owner_client, input_data, community_1).json["id"]
...
def test_reader(
    ...
    resp_reader = _check_community_submission_and_publish(reader_client, reader_client, community_owner, rec_id, community_2.data["id"])
    resp_curator = _check_community_submission_and_publish(reader_client, curator_client, community_curator, rec_id,
                                            community_2.data["id"])
